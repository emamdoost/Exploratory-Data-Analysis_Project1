...
par(mfrow = c(2, 2))
GAPower <- as.numeric(as.character(subSetRows$Global_active_power))
plot(strptime(paste(subSetRows$Date, subSetRows$Time, sep=" "), "%d/%m/%Y %H:%M:%S"), 
     GAPower, type="l", xlab="", ylab="Global Active Power", cex=0.2)

plot(strptime(paste(subSetRows$Date, subSetRows$Time, sep=" "), "%d/%m/%Y %H:%M:%S"), 
     as.numeric(as.character(subSetRows$Voltage)), type="l", xlab="datetime", ylab="Voltage")
...
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), 
       col=c("black", "red", "blue"), lty=1, bty="n", cex=.5)

plot(strptime(paste(subSetRows$Date, subSetRows$Time, sep=" "), "%d/%m/%Y %H:%M:%S"), 
     as.numeric(as.character(subSetRows$Global_reactive_power)), type="l", xlab="datetime", ylab="Global_reactive_power")
...
## Process is finished.
message("The process of script is finished and plot4.png is created.")  
